...
def run_offset_sensor_fdr(
    ...
    """
    # média temporal da mesma janela usada no FDR
    topo_data = evoked.data[:, win].mean(axis=1)

    # grads: reduzir máscara 204 -> 102 pares
    if method == "grad":
        mask_plot = reject.reshape(-1, 2).any(axis=1)
    else:
        mask_plot = reject
    ...
    fig.suptitle(
        ...
        f"Mean offset response {tmin:.2f}-{tmax:.2f} s (FDR q<{alpha})",
        fontsize=12,
    )

    # mostrar apenas uma figura
    plt.show()
