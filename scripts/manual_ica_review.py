...
#PRECISAS DE FAZER ISTO FUNCIONAR PARA O CASO DE NAO HAVER EEG
...
#
# Load previously trained MEG and EEG ICA solutions.
# The EEG ICA file may not exist for some subjects
# (e.g. subjects without EEG data), so we check first.
...
if ica_meg_path.exists():
    ica_meg = read_ica(ica_meg_path)
else:
    print(f"[WARNING] MEG ICA file not found: {ica_meg_path}")

if ica_eeg_path.exists():
    ica_eeg = read_ica(ica_eeg_path)
else:
    print(f"[INFO] EEG ICA file not found for subject {subject}. "
          f"Skipping EEG ICA inspection.")
    print(f"       Expected path: {ica_eeg_path}")
