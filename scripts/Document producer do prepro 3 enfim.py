#Document producer do prepro 3 enfim
...
# Primeiro loop: adiciona apenas os raw viewers
raws = []  # guardar os objetos raw para usar depois
for path in file_paths:
    raw = mne.io.read_raw_fif(path, preload=True)
    raw_qc = raw.copy()
    raw_qc.info["bads"] = []
    raws.append(raw_qc)
    # Adiciona o raw viewer com título diferenciado (opcional)
    report.add_raw(raw=raw_qc, title="Raw data viewer", psd=False)

# Segundo loop: adiciona todos os outros conteúdos (com section)
for raw_qc, name in zip(raws, names):
    # ... todo o resto (add_html, add_figure com section=name) ...
    ...
    fig = raw_qc.compute_psd().plot(
        picks="data",          # plota todos os canais de dados (mag, grad, eeg)        # exclui os canais marcados como ruins
        amplitude=False,       # se quiser amplitude em vez de potência, mude para True
        show=False             # evita que a figura seja exibida agora (opcional)
    )

    # Adiciona a figura ao relatório
    report.add_figure(
        fig,
        title="PSD plot",      # título que aparece no relatório
        section=name           # seção onde vai ficar
    )
    ...
    try:
        ...
        # calcula posição da cabeça a partir das HPI coils
        ...
        # calcular deslocamento relativo à posição inicial
        pos = head_pos[:, 1:4]   # x,y,z
        pos0 = pos[0]
        ...
        ax.set_title("Head displacement over time")
