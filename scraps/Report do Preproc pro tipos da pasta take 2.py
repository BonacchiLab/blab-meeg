# Report do Preproc pro tipos da pasta take 2
...
for raw, name in zip(raws, names):
    ...
    report.add_figure(
        fig_psd,
        title=f"{name} - PSD after Maxwell",
        section=name
    )
    ...
    report.add_raw(
        raw_sss,
        title=f"{name} - Raw after Maxwell",
        section=name
    )
...
report.add_figure(fig, title="ICA timecourses", section="ICA")

# ---------------------------------------------------------
# BEFORE/AFTER ICA (run 1)
# ---------------------------------------------------------
...
report.add_figure(fig,title="ICA comparison",section="ICA")
