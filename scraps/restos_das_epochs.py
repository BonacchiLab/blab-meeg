...
def run_epoch_offset_creator_IM_TESTING_ANOTHER_ONE(subject, method, crop):
...
def run_epoch_offset_creator(
    ...
    epochs_500.shift_time(
        tshift=-0.5,
        relative=True,
    )

    epochs_1000.shift_time(
        tshift=-1.0,
        relative=True,
    )

    epochs_1500.shift_time(
        tshift=-1.5,
        relative=True,
    )
    ...
    #
    # 500 ms stimulus:
    #   baseline = -0.7 to -0.5 s
    #
    # 1000 ms stimulus:
    #   baseline = -1.2 to -1.0 s
    #
    # 1500 ms stimulus:
    #   baseline = -1.7 to -1.5 s
    #
