...
def run_prep_pipeline(
    ...
    for i, (prep, raw_sss, raw_prep, raw_prepped, raw_clean) in enumerate(
        zip(preps, raws_sss, raws_prep, raws_prepped, raws_clean)
    ):
        run_name = names[i]
        ...
        prep_info_run = {
            ...
            "reference": {"ref_chs": "eeg", "reref_chs": "eeg"},
        }
        ...
        if len(prep_bads) > 0:
            ...
            report.add_figure(
                fig_bads_before,
                title=f"Bad EEG channels - before PREP - {run_name}",
                section=run_name,
            )
            report.add_figure(
                fig_bads_after,
                title=f"Bad EEG channels - after PREP - {run_name}",
                section=run_name,
            )
            report.add_figure(
                fig_bads_after_final,
        ...
        report.add_figure(
            fig_psd_prep, title=f"PSD after Prep - {run_name}", section=run_name
        )
        report.add_figure(
            fig_psd_prepped,
            title=f"PSD after final interpolation - {run_name}",
            section=run_name,
        )
        ...
        fig_eeg_raw = (
            raw_sss.copy()
            .pick("eeg")
            .plot(duration=10, start=50, butterfly=True, show=False)
        )
        fig_eeg_prep = raw_prep.copy().plot(
            duration=10, start=50, butterfly=True, show=False
        )
        fig_eeg_prepped = raw_prepped.copy().plot(
            duration=10, start=50, butterfly=True, show=False
        )

        report.add_figure(fig_eeg_raw, title=f"EEG Raw - {run_name}", section=run_name)
        report.add_figure(
            fig_eeg_prep, title=f"EEG after Prep - {run_name}", section=run_name
        )
        report.add_figure(
            fig_eeg_prepped,
            title=f"EEG after final interpolation - {run_name}",
            section=run_name,
        )
