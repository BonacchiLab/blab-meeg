...
def run_preprocess_epochs_p2(
    ...
    fig_evoked_baseline = epochs_baselined.average().plot(show=False)
    fig_evoked_clean = epochs_clean.average().plot(show=False)
    fig_evoked_final = epochs_final.average().plot(show=False)
    ...
    report.add_figure(fig_evoked_baseline, title="Evoked after baseline correction")
    report.add_figure(fig_evoked_clean, title="Evoked after reject criteria")
    report.add_figure(fig_evoked_final, title="Evoked after final filtering")
    # =========================
    # 5) METADATA 🔥
    # =========================
    ...
    epochs_final.save(file_path, overwrite=True)
...
if __name__ == "__main__":
    ...
    # --- caminhos dos ficheiros ---
    file_paths = [
        rf"{sub_dur_outdir}\{subject}_03_ica__preprocessed.fif",
    ]
