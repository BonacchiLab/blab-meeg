...
def run_badch_maxwell(
    ...
    for i, (raw, raw_badch, raw_sss) in enumerate(zip(raws, raws_badch, raws_sss)):
        ...
        for ch_type in channel_types:
            ...
            for cond in conditions:
                ...
                for x in range(1, len(bins)):
                    ax[0].axvline(
                        x, ls="dashed", lw=0.25, dashes=(25, 15), color="gray"
                    )
        ...
        if len(bads) > 0:
            ...
            report.add_figure(
                fig_bads_raw, title="Bad channels - RAW", section=run_name
            )
            report.add_figure(
                fig_bads_sss, title="Bad channels - after SSS", section=run_name
            )
        ...
        report.add_figure(
            fig_psd_sss_mag, title="PSD after Maxwell - mag", section=run_name
        )
        report.add_figure(fig_psd_raw_grad, title="PSD Raw - grad", section=run_name)
        report.add_figure(
            fig_psd_sss_grad, title="PSD after Maxwell - grad", section=run_name
        )
        ...
        fig_meg_raw = (
            raw.copy()
            .pick(["meg"])
            .plot(duration=10, start=50, butterfly=True, show=True)
        )
        fig_meg_sss = (
            raw_sss.copy()
            .pick(["meg"])
                ...
                rows.append(
                    {
                        ...
                        "mean_noisy": np.nanmean(
                            auto_scores["scores_noisy"][ch_subset][j]
                        ),
                        "max_noisy": np.nanmax(
                            auto_scores["scores_noisy"][ch_subset][j]
                        ),
                        "mean_flat": np.nanmean(
                            auto_scores["scores_flat"][ch_subset][j]
                        ),
